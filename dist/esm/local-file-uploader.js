    ...
    async uploadFilesFromRequest(req, field, container) {
        ...
        const files = await new Promise((resolve, reject) => {
            ...
            form.parse(req, (err, fields, files) => {
                if (err)
                    return;
                const uploadedFiles = [].concat(files[field] || []);
        ...
        try {
            for (const file of files) {
                ...
                if (!fileType) {
                    throw createError(HttpErrors.BadRequest, 'INVALID_FILE', localizer.t(`${errorKeyPrefix}.invalidFileError`), { originalFileName });
                }
