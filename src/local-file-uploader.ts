 ...
 */
export const LOCAL_FILE_UPLOADER_DEFAULT_OPTIONS: Flatten<
  Required<LocalFileUploaderOptions>
> = {
  targetDir: 'upload',
...
export class LocalFileUploader extends DebuggableService {
   ...
   * Options.
   */
  options: Flatten<Required<LocalFileUploaderOptions>> = structuredClone(
  ...
  constructor(
    ...
    if (this.hasService(LocalFileUploaderOptions)) {
      const optionsService = this.getRegisteredService(
        LocalFileUploaderOptions,
      );
      const definedOptions = Object.fromEntries(
        Object.entries(optionsService).filter(([, value]) => value != null),
      );
  ...
  async uploadFilesFromRequest(
        ...
        if (!uploadedFiles.length) {
          return reject(
            createError(
              HttpErrors.BadRequest,
              'NOTHING_TO_UPLOAD',
              localizer.t(`${errorKeyPrefix}.nothingToUploadError`),
            ),
          );
        }
    ...
    debug(
      numWords(files.length, '%d file received.', '%d files received.'),
      files.length,
    );
    files.forEach(file => {
    ...
    try {
      for (const file of files) {
        ...
        if (
          ...
          if (canCreateThumbs) {
            const thumbnails = await thumbnailService.createThumbnails(
              finalFilePath,
              resourceDir,
